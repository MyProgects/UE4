...
// MOBAEngine change begin
// add by xxh
// 避免多线程加载时不必要的多次锁
COREUOBJECT_API TArray<int32> LoadPackagesAsync(const TArray<FString>& InNames, const FGuid* InGuid, FName InType, const TCHAR* InPackageToLoadFrom = nullptr, FLoadPackageAsyncDelegate InCompletionDelegate = FLoadPackageAsyncDelegate(), EPackageFlags InPackageFlags = PKG_None, int32 InPIEInstanceID = INDEX_NONE, TAsyncLoadPriority InPackagePriority = 0);
COREUOBJECT_API TArray<int32> LoadPackagesAsync(const TArray<FString>& InNames, const FGuid* InGuid = nullptr, const TCHAR* InPackageToLoadFrom = nullptr, FLoadPackageAsyncDelegate InCompletionDelegate = FLoadPackageAsyncDelegate(), EPackageFlags InPackageFlags = PKG_None, int32 InPIEInstanceID = INDEX_NONE, TAsyncLoadPriority InPackagePriority = 0);
COREUOBJECT_API TArray<int32> LoadPackagesAsync(const TArray<FString>& InNames, FLoadPackageAsyncDelegate InCompletionDelegate, TAsyncLoadPriority InPackagePriority = 0, EPackageFlags InPackageFlags = PKG_None);
// MOBAEngine change end
...
struct COREUOBJECT_API FCoreUObjectDelegates
{
	...
	// Modified by xxh
	/*
	如果在PreLoadMap中调用PlayMovie，由于主线程和Loading UI线程共同执行，主线程释放资源，UI线程在使用，导致Crash
	*/
	// LoadMap之前，Pre Map GC之后
	static FSimpleMulticastDelegate BeginLoadMap;
	// MOBAEngine change end
